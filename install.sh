...
manageCDN() {
    ...
    if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,|,11,"; then
        ...
        echoContent yellow "# 注意事项"
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "https://www.FreeShell.cc"
        ...
        echoContent yellow "\n教程地址:"
        echoContent skyBlue "https://FreeShell.cc"
...
ipv6Routing() {
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# 注意事项\n"
        echoContent yellow "# 注意事项"
        echoContent yellow "# 使用教程：FreeShell.cc \n"
...
warpRoutingReg() {
    ...
    if [[ "${warpStatus}" == "1" ]]; then
        ...
        echoContent yellow "# 使用教程：https://FreeShell.cc \n"
...
addOtherSubscribe() {
    echoContent yellow "#注意事项:"
    echoContent yellow "请仔细阅读以下文章： https://FreeShell.cc"
    echoContent skyBlue "录入示例：FreeShell.cc:443:vps1\n"
...
initRealityClientServersName() {
    ...
    if [[ -z "${realityServerName}" ]]; then
        ...
        if [[ -z "${realityServerName}" ]]; then
            ...
            echoContent skyBlue "\n================ 配置客户端可用的serverNames ===============\n"
            echoContent yellow "#注意事项"
            echoContent green "Reality目标可用域名列表：https://FreeShell.cc\n"
...
manageReality() {
    ...
    if ! echo "${currentInstallProtocolType}" | grep -q -E "7,|8," || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> 请先安装Reality协议，参考教程 https://FreeShell.cc"
        exit 0
    fi
...
menu() {
    cd "$HOME" || exit
    echoContent red "\n=============================================================="
    echoContent green "作者：法喜"
    echoContent green "当前版本：v3.5.18"
    echoContent green "Github：https://FreeShell.cc"
    echoContent green "描述：八合一共存脚本\c"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n=========================== 推广区============================"
    echoContent red "                                              "
    echoContent yellow "梯子专用VPS选购"
    echoContent green "https://FreeShell.cc"
